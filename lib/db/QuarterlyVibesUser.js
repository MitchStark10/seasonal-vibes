...
export class QuarterlyVibesUser {
  ...
  async getSettings() {
    ...
    const subscriptionResponsePromise = this.isSubscribed
      ? Promise.resolve({ resultSet: [{ IsSubscribed: true }] })
      : connectAndQuery(GET_SUBSCRIPTION_SQL, [this.emailQueryParam], {
          logInfo: true,
        });
