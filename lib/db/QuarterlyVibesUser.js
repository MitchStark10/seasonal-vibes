...
SET IsSubscribed = @isSubscribed,
  PlaylistVisibilityType = @playlistVisibilityType
WHERE Email = @email
`;
...
export class QuarterlyVibesUser {
  ...
  async getSettings() {
    ...
    this.settingsCache = { isSubscribed, playlistVisibilityType };
  ...
  async updateSettings({ isSubscribed, playlistVisibilityType }) {
    return connectAndQuery(UPDATE_SETTINGS_SQL, [
      {
        key: "isSubscribed",
        type: sql.Bit,
        value: isSubscribed,
      ...
      {
        key: "playlistVisibilityType",
        type: sql.VarChar,
        value: playlistVisibilityType,
      },
      this.emailQueryParam,
    ]);
  }
