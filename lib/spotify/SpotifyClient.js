...
export class SpotifyClient {
  ...
  async getTopTracks(quantity, genresToFilter = []) {
    await this.accessTokenPromise;

    console.log("Fetching top tracks for user: ", {
      userId: this.userId,
      quantity,
    });
    const response = await got(
      `https://api.spotify.com/v1/me/top/tracks?limit=${quantity}&time_range=short_term`,
      {
        headers: this.defaultHeaders,
      }
    ).json();
    ...
    return response.items.filter(item => {
      for (const currentSongGenre of item.genres) {
        if (genresToFilter.includes(currentSongGenre)) {
          return false;
        }
      }

      return true;
    }).slice(0, quantity).map((item) => ({
  ...
  async createPlaylist({ playlistName, manualRequestForNewPlaylist }) {
    ...
    if (!manualRequestForNewPlaylist) {
  ...
  async createScheduledPlaylist({
    playlistName,
    manualRequestForNewPlaylist = false,
    quantityOfSongsPerPlaylist,
  }) {
    if (!this.userEmail) {
      throw new Error("Cannot create a playlist without email context");
    } else if (!quantityOfSongsPerPlaylist) {
      throw new Error("Missing required parameter quantityOfSongsPerPlaylist");
    }
    ...
    const tracks = await this.getTopTracks(quantityOfSongsPerPlaylist);
    const playlistId = await this.createPlaylist({
      playlistName,
      manualRequestForNewPlaylist,
    });
